...
use App\Unifin\Repositories\TodayTotals\TodayTotalsInterface;
...
//        dd($groupName, $gft, $thisMonth, $thirtyDays, $all);
    ...
    public function getData(TodayTotalsInterface $todayTotals)
    {
        if (request()->wantsJson()) {

            $groups = $todayTotals->groups();
            $gft = $todayTotals->gft();
            $thisMonth = $todayTotals->thisMonth();
            $thirtyDays = $todayTotals->thirtyDays();
            $all = $todayTotals->all();

            return compact('groups', 'gft', 'thisMonth', 'thirtyDays', 'all');
        }
    }
}