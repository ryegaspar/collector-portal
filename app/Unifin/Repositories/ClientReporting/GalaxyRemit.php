...
class GalaxyRemit implements ReportInterface
{
    ...
    public function generateReport($request) 
    {
        ...
        $data = DB::connection('sqlsrv2')
                ...
                ->select(DB::raw("DBR_CLI_REF_NO, DBR_NAME1, DBR_NO, TRS_AMT, TRS_COMM_AMT, TRS_TRX_DATE_O,TRS_TRUST_CODE,DBR_STATUS"))
                ->whereNotIn('TRS_TRUST_CODE', ['14','2', '3', '33'])
                ->whereRaw('DBR_CLIENT LIKE ?', ['GXY%'])
                ->where('TRS_AMT', '<>', 0)
        ...
        $fileName = 'Galaxy_REM_'. Carbon::now()->format('Ymd') . '.xlsx';
